import Link from "next/link";

export default function Home() {
  return (
    <div className="flex flex-1 items-center justify-center bg-gray-100 p-6">
      <main className="w-full max-w-md rounded-xl bg-white p-8 shadow sm:max-w-lg sm:p-10">
        <p className="text-3xl font-semibold text-blue-700">LogiSync</p>
        <h1 className="mt-3 text-xl font-semibold tracking-tight text-zinc-900 sm:text-2xl">
          Freight and digital waybill management
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-zinc-600 sm:text-base">
          Create digital waybills, price Central America routes, and manage your
          driver directory from one place built for freight ops.
        </p>

        <ul className="mt-6 space-y-2 border-t border-zinc-100 pt-6 text-sm text-zinc-700">
          <li>Digital waybills with printable PDFs</li>
          <li>Route pricing with real distance and margins</li>
          <li>Driver directory filtered by preferred cities</li>
        </ul>

        <Link
          href="/login"
          className="mt-8 inline-flex w-full items-center justify-center rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-blue-700 sm:w-auto"
        >
          Sign in
        </Link>
      </main>
    </div>
  );
}
